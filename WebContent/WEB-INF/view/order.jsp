...
<body>
		...
		<div class="mypage_menu">
			<div class="myMenu">
				<ul class="baseBox">
					...
					<li><a href="${root}userlike">관심상품</a></li>
	...
	<form action="${root }payment" method="post" id="payment"
				...
				<table class="shopping_cart">
					...
					<tbody>
						...
						<c:if test="${not empty OrdDto }">
							<c:set var="cal1"
								value="${cal1 + OrdDto.ordPrice*OrdDto.ordCount }" />

							<tr>
								<td><a href="#"><img
										src="${root }images/${OrdDto.thumbImg }" alt="장바구니_상품3"></a></td>
								<td>${OrdDto.gdsNum }:${OrdDto.gdsName }<br> <a
									class="cartOption"> [옵션] color : ${OrdDto.ordColor} size :
										${OrdDto.ordSize}</a></td>
						...
						<c:forEach var="dto" items="${CartList }">
							...
							<tr>
								...
								<td><a href="#"><img
										src="${root }images/${dto.thumbImg }" alt="장바구니_상품3"></a></td>
								<td>${dto.gdsName}:${dto.gdsNum }<br> <a
									class="cartOption">[옵션] color : ${dto.cartColor } size :
										${OrdDto.ordSize}</a></td>
						...
						<tr>
							<td><input type="hidden" value="${OrdDto.gdsNum }"
								name="gdsNum" /> <input type="hidden"
								value="${OrdDto.gdsName }" name="ordName" /> <input
								type="hidden" value="${OrdDto.ordColor }" name="ordColor" /> <input
								type="hidden" value="${OrdDto.ordCount }" name="ordCount" /> <!-- 추가 -->
								<input type="hidden" value="${OrdDto.ordPrice }" name="ordPrice" />
								<input type="hidden" value="${user.userNum }" name="userNum" />
								<input type="hidden" value="${user.email }" name="email" /> <input
								type="hidden" name="payMethod" value="무통장입금"> <input
								type="hidden" id="bankName" name="bankName" value="" /> <!-- 넘기기 금액, 쿠폰, 누적적립금, 사용적립금 -->

								<input type="hidden" name="cal" id="cal" value="${cal1+cal2 }">
								<fmt:parseNumber var="point" value="${(cal1+cal2)*0.02 }"
									integerOnly="true" /> <input type="hidden" name="point"
								id="point" value="${point}"> <c:set var="usePoint"
									value="0" /> <!-- 추가 --> <c:set var="useCouponCal" value="0" />
								<input type="hidden" name="usePoint" id="usePoint"
								value="${usePoint }"> <!-- 추가 --> <input type="hidden"
								name="useCouponCal" id="useCouponCal" value="${useCouponCal }">
								<input type="hidden" name="cpNum" id="useCp" class="useCp"
								value="0"></td>
						</tr>
					...
					<tfoot class="cartFootLine">
						<tr>
							<td colspan="8">
								<div class="cartFoot">
									총 구매금액 : <strong><fmt:formatNumber
											value="${cal1+cal2 }" type="number" />원</strong> (적립금 :
			...
			<table class="tbl_orderTbl">
				...
				<tbody>
					<tr>
						...
						<td>${user.name }<input type="hidden" id="name"
							class="MS_input_txt" value="${user.name }">
						</td>
					</tr>
					<tr>
					</tr>
					...
					<tr>
						<th>연락처</th>
						<td>${user.tel }<input type="hidden" id="tel" size="4"
							maxlength="4" class="MS_input_txt w60" value="${user.tel }">
						</td>
					</tr>
				</tbody>
			</table>
			<input type="hidden" id="mAddr" size="4" maxlength="4"
				class="MS_input_txt w60" value="${user.mAddr }"> <input
				type="hidden" id="lAddr" size="4" maxlength="4"
				class="MS_input_txt w60" value="${user.lAddr }"> <input
				type="hidden" id="dlAddr" size="4" maxlength="4"
				class="MS_input_txt w60" value="${user.dlAddr }">
		</div>
					...
					<table class="payment">
						...
						<tbody>
							...
							<tr height="100">
								...
								<td><div class="base">
										<span><a class="plus"><img
												src="${root }icon/ic_plus.png" alt="plus"></a>무료</span>
									</div></td>
								<td><div class="base">
										<span><a class="minus"><img
												src="${root }icon/ic_minus.png" alt="minus"></a><span
											id='couponView'> 0</span></span>원
									</div></td>
								<td><div class="base">
										<span><a class="minus"><img
												src="${root }icon/ic_minus.png" alt="minus"></a><span
											id='pointView'> 0</span></span>원
									</div></td>
								<td><div class="base">
										<span><a class="plus"><img
												src="${root }icon/ic_equal.png" alt="equal"></a><span
											id="calView" class="redfont"><fmt:formatNumber
													value="${cal1+cal2 }" type="number" /></span></span>원
						...
						<tfoot>
							<tr>
								...
								<td colspan="4" style="text-align: left"><select
									style="width: 212px;" id="cpSelect" name="cpSelect"
									...
									class="w280 MK_bank_select_list MK_pay_add_choice">
										...
										<c:forEach var="cl" items="${cpList }" varStatus="status">
											...
											<fmt:parseNumber value="${cl.cpRate*cal}" integerOnly="true" />

											<option value="${cl.cpRate }|${cl.cpNum } ">
												${cl.cpName } ${cl.cpRate }% 할인 쿠폰</option>
										</c:forEach>
								</select> <input type="button" class="button"
									onclick="getInputValue2(); calCulation(smart); getInputValue2();"
									value="적용"></td>
							<tr>

								<td style="font-size: 1.2em; font-weight: bold;">적립금</td>
								<td colspan="4" style="text-align: left">
									<!-- 적립금 입력창 숫자만 입력가능 --> <input type="text" id="couponnum"
									class="inputPointClass" name="inputPointClass"
									onkeypress="inNumber();" value="0"> <input
									type="button" class="button" onclick="minPoint();" value="사용">

									<c:set var="tp" value="${totalPoint}" /> <c:choose>
     ...
     보유적립금 : 0원
       ...
       보유적립금 : ${totalPoint.totalPoint }원
							...
							<tr class="cartFootline2">
								<td colspan="8">
									<div class="cartFoot">적립금은 100원 단위로 사용하실 수 있습니다. 구매금액의
										2%가 적립됩니다.</div>
								</td>
							</tr>
				...
				<table class="tbl_endPriceTbl">
					...
					<tbody>
						...
						<tr class="endPrice" height="100px">
							...
							<td><strong><span id="LastCalView" class="redfont"><fmt:formatNumber
											value="${cal1+cal2 }" type="number" /></span></strong> 원 &nbsp; (적립예정: <span
								id="lastPoint"><fmt:formatNumber
	...
	<!-- 쿠폰적용, 적립금 계산기 화면 -->
	<input type="hidden" id="inputCpClass" class="inputCpClass"
		name="inputCpClass" value="0">
	...
	<div>
		<form id="smart" onload="calCulation(smart);">
			<input type='hidden' name='first' size='7' value="${cal1+cal2 }"
				onKeyUp=calCulation(smart); readonly> <input type='hidden'
				name='second' size='7' id="calPoint" value="0"
				onKeyUp=calCulation(smart);> <input type='hidden'
				name='third' size='7' id="calCoupon" value="0"
				onKeyUp=calCulation(smart);> <input type='hidden'
				name='cpCal' class="cpCal" size='7' value="0" readonly> <input
				type='hidden' name='pointTotal' class="pointTotal" size='7'
				value="0" readonly> <input type='hidden' name='totalCal'
				class="totalCal" size='7' value="${cal1+cal2 }">
